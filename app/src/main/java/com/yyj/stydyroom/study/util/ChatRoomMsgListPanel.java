...
import com.yyj.stydyroom.base.ui.dialog.EasyAlertDialog;
import com.yyj.stydyroom.base.ui.dialog.EasyAlertDialogHelper;
import com.yyj.stydyroom.base.ui.listview.AutoRefreshListView;
import com.yyj.stydyroom.base.ui.listview.ListViewUtil;
import com.yyj.stydyroom.base.ui.listview.MessageListView;
import com.yyj.stydyroom.study.session.Container;
import com.yyj.stydyroom.study.session.adapter.MsgAdapter;
...
import com.yyj.stydyroom.study.viewholder.ChatRoomMsgViewHolderFactory;
...
//        MessageAudioControl.getInstance(container.activity).stopAudio(); // 界面返回，停止语音播放
    ...
    public void onIncomingMessage(List<ChatRoomMessage> messages) {
        ...
        MsgAttachment attachment = messages.get(0).getAttachment();
        ...
        if (attachment instanceof ChatRoomNotificationAttachment) {
            ...
            if (notificationAttachment.getExtension() != null &&
                    notificationAttachment.getExtension().containsKey(MeetingConstant.SHOW_TYPE)) {
                return;
            }
        }
    ...
    private void saveMessage(final IMMessage message, boolean addFirst) {
        if (message == null || needFilter(message)) {
            return;
        }
    ...
    private boolean needFilter(final IMMessage message) {
        ...
        if (msgAttachment instanceof PermissionAttachment) {
            return true;
        }

        if (msgAttachment instanceof ChatRoomNotificationAttachment) {
            ChatRoomNotificationAttachment attachment = (ChatRoomNotificationAttachment) message.getAttachment();
            return (attachment != null &&
                    attachment.getExtension() != null &&
                    attachment.getExtension().containsKey(MeetingConstant.SHOW_TYPE));
        }
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void showReDownloadConfirmDlg(final IMMessage message) {
            ...
            final EasyAlertDialog dialog = EasyAlertDialogHelper.createOkCancelDialog(container.activity, null,
